<template>
    <div :class="`${horizontal ? 'flex' : ''}  ${merged ? 'merged' : ''}`">
      ...
      }  inline-block input-label `" :for="name">
            {{ label }}</label>
        <div class="flex items-stretch inputGroup" :class="`
    ...
    ${horizontal ? 'flex-1' : ''}
    `">
            <span class="flex-none input-group-addon" v-if="prepend || prependIcon || $slots.prepend">
                <span class="input-group-text inline-block h-full" v-if="prepend || prependIcon">
                    {{ prepend }}
            ...
            <div class="flex-1">
                <div class="relative fromGroup2" :class="`${error ? 'has-error' : ''}  ${validate ? 'is-valid' : ''}`">
                    <input :type="types" :name="name" :placeholder="placeholder" :class="`${classInput} input-group-control block w-full focus:outline-none h-[40px] `" :value="modelValue" @input="$emit('update:modelValue', $event.target.value)" :error="error" :id="name" :readonly="isReadonly" :disabled="disabled" :validate="validate" v-if="!isMask" />
                    <cleave :class="`${classInput} cleave input-group-control block w-full focus:outline-none h-[40px] `" :name="name" :placeholder="placeholder" :value="modelValue" @input="$emit('update:modelValue', $event.target.value)" :error="error" :id="name" :readonly="isReadonly" :disabled="disabled" :validate="validate" :options="options" v-if="isMask" modelValue="modelValue" />
                    <div class="flex text-xl absolute ltr:right-[14px] rtl:left-[14px] top-1/2 -translate-y-1/2" v-if="error || validate">
            ...
            <span class="flex-none input-group-addon right" v-if="append || appendIcon || $slots.append">
                <span class="input-group-text inline-block h-full" v-if="append || appendIcon">{{ append }}
        ...
        <span v-if="error" class="mt-2" :class="
        msgTooltip
          ? ' inline-block bg-danger-500 text-white text-[10px] px-2 py-1 rounded'
          : ' text-danger-500 block text-sm'
      ">{{ error }}</span>
        <span v-if="validate" class="mt-2" :class="
        msgTooltip
          ? ' inline-block bg-success-500 text-white text-[10px] px-2 py-1 rounded'
          : ' text-success-500 block text-sm'
      ">{{ validate }}</span>
        <span class="block text-slate-400 font-light leading-4 text-xs mt-2" v-if="description">{{ description }}</span>
    </div>
</template>
<script>
